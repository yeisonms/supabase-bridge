...
CREATE OR REPLACE FUNCTION "public"."can_user_review"("p_partner_id" "uuid") RETURNS boolean
...
ALTER FUNCTION "public"."can_user_review"("p_partner_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."confirm_access"("p_checkin_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_result RECORD;
BEGIN
  -- Ahora sí funcionará porque la columna ya existe
  UPDATE checkins 
  SET status = 'confirmed', updated_at = now()
  WHERE id = p_checkin_id AND status = 'reserved'
  RETURNING id INTO v_result; -- Guardamos el resultado en una variable
  
  -- Verificamos si se actualizó algo
  IF v_result IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'Error: La reserva ya no es válida o ya se usó.');
  ELSE
    RETURN json_build_object('success', true, 'message', 'Acceso confirmado exitosamente.');
  END IF;
END;
$$;


ALTER FUNCTION "public"."confirm_access"("p_checkin_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_checkin_profiles"("p_user_ids" "uuid"[]) RETURNS TABLE("id" "uuid", "first_name" "text", "last_name" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
ALTER FUNCTION "public"."get_checkin_profiles"("p_user_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_nearby_partners"("lat" double precision, "long" double precision, "radius_meters" integer DEFAULT 5000) RETURNS TABLE("id" "uuid", "name" "text", "address" "text", "category" "text", "lat" double precision, "long" double precision, "dist_meters" double precision, "image_url" "text", "daily_capacity_limit" integer, "is_active" boolean)
    LANGUAGE "sql"
    AS $$
  select 
    id,
  ...
  from 
    public.partners
  where 
    st_dwithin(location, st_point(long, lat)::geography, radius_meters)
    and is_active = true
  order by 
    dist_meters asc; -- Ordena del más cercano al más lejano
$$;


ALTER FUNCTION "public"."get_nearby_partners"("lat" double precision, "long" double precision, "radius_meters" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."notify_reservation"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_user_email text;
...
BEGIN
  ...
  -- Enviar solo si es reserva nueva
  IF NEW.status = 'reserved' THEN
    PERFORM send_email_resend(
      v_user_email,
      'Confirmación: Reserva en ' || v_partner_name,
      '<h1>¡Reserva Exitosa!</h1><p>Has asegurado tu lugar en <strong>' || v_partner_name || '</strong>.</p><p>Presenta tu QR al llegar.</p>'
    );
  END IF;
  RETURN NEW;
END;
...
ALTER FUNCTION "public"."notify_reservation"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."partner_scan_user"("p_user_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_partner_admin_id UUID := auth.uid(); -- El usuario logueado (Dueño del Gym)
  v_partner_id UUID;
  v_plan_level INT;
  v_min_level INT;
  v_capacity INT;
  ...
  v_user_status TEXT;
BEGIN
  -- 1. Identificar qué Gimnasio es el que está escaneando
  SELECT id, min_plan_level, daily_capacity_limit, is_active 
  INTO v_partner_id, v_min_level, v_capacity, v_user_status -- reciclamos variable status para check de gym
  FROM public.partners 
  WHERE admin_user_id = v_partner_admin_id;

  IF v_partner_id IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'Error: No tienes un gimnasio asociado a tu cuenta.');
  END IF;

  -- 2. Validar al USUARIO Escaneado (p_user_id)
  SELECT p.access_level, pr.subscription_status 
  INTO v_plan_level, v_user_status
  FROM public.profiles pr
  JOIN public.plans p ON pr.current_plan_id = p.id
  WHERE pr.id = p_user_id;

  IF v_user_status IS DISTINCT FROM 'active' THEN
    RETURN json_build_object('success', false, 'message', 'El usuario no tiene una suscripción activa.');
  END IF;

  -- 3. Validar Nivel de Acceso (Plan del Usuario vs Nivel del Gym)
  IF v_plan_level < v_min_level THEN
    RETURN json_build_object('success', false, 'message', 'Plan insuficiente para este gimnasio.');
  END IF;

  -- 4. Validar si ya entró hoy (Regla de Unicidad)
  -- Esto evita que se pasen el QR o entren doble vez
  IF EXISTS (SELECT 1 FROM public.checkins WHERE user_id = p_user_id AND partner_id = v_partner_id AND checkin_date = CURRENT_DATE) THEN
     RETURN json_build_object('success', false, 'message', 'Este usuario ya ingresó hoy.');
  END IF;

  -- 5. Registrar la Visita
  INSERT INTO public.checkins (user_id, partner_id, checkin_date)
  VALUES (p_user_id, v_partner_id, CURRENT_DATE);

  RETURN json_build_object('success', true, 'message', 'Acceso Autorizado. ¡Bienvenido!');
END;
$$;


ALTER FUNCTION "public"."partner_scan_user"("p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."process_checkin"("p_partner_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  ...
  v_count INT;
BEGIN
  -- 1. Verificar suscripción activa y obtener nivel del plan
  SELECT p.access_level INTO v_plan_level
  FROM profiles pr 
  JOIN plans p ON pr.current_plan_id = p.id
  WHERE pr.id = v_user_id AND pr.subscription_status = 'active';
  
  IF v_plan_level IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'No tienes un plan activo. Ve a Planes para suscribirte.');
  END IF;

  -- 2. Verificar datos del gimnasio
  SELECT min_plan_level, daily_capacity_limit INTO v_min_level, v_capacity
  FROM partners 
  WHERE id = p_partner_id AND is_active = true;

  IF v_min_level IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'Gimnasio no encontrado o inactivo.');
  END IF;

  -- 3. Comparar Niveles (¿Mi plan cubre este gym?)
  IF v_plan_level < v_min_level THEN
    RETURN json_build_object('success', false, 'message', 'Tu plan actual no cubre este gimnasio. ¡Mejora tu plan!');
  END IF;

  -- 4. Validar Aforo Diario (Si aplica)
  IF v_capacity IS NOT NULL THEN
    SELECT COUNT(*) INTO v_count 
    FROM checkins 
    WHERE partner_id = p_partner_id AND checkin_date = v_today;
    
    IF v_count >= v_capacity THEN
      RETURN json_build_object('success', false, 'message', 'Cupo lleno en este gimnasio por hoy.');
    END IF;
  END IF;

  -- 5. Intentar Insertar el Check-in
  BEGIN
    INSERT INTO checkins (user_id, partner_id, checkin_date) 
    VALUES (v_user_id, p_partner_id, v_today);
    
    RETURN json_build_object('success', true, 'message', '¡Check-in exitoso! A entrenar.');
  EXCEPTION WHEN unique_violation THEN
    -- Aquí atrapamos el error si intenta entrar dos veces
    RETURN json_build_object('success', false, 'message', 'Ya registraste tu visita en este gimnasio hoy.');
  END;
END;
$$;


ALTER FUNCTION "public"."process_checkin"("p_partner_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."process_checkin"("gym_id" "uuid", "user_location_lat" double precision, "user_location_long" double precision) RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
BEGIN
  ...
  -- VALIDACIÓN 1: ¿Tiene plan activo?
  IF user_profile.subscription_status != 'active' OR user_profile.current_plan_id IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'No tienes una suscripción activa. Por favor compra un plan.');
  END IF;

  -- B. Obtener datos del plan
  SELECT * INTO user_plan FROM public.plans WHERE id = user_profile.current_plan_id;

  -- C. Obtener datos del gimnasio
  SELECT * INTO gym_record FROM public.partners WHERE id = gym_id;

  -- VALIDACIÓN 2: ¿El plan cubre este gimnasio?
  -- Asumimos que 'access_level' en planes y 'min_plan_level' en partners son enteros (1, 2, 3)
  IF user_plan.access_level < gym_record.min_plan_level THEN
    RETURN json_build_object('success', false, 'message', 'Tu plan actual no cubre este gimnasio. ¡Mejora tu plan para acceder!');
  END IF;

  -- VALIDACIÓN 3: Geolocalización (Opcional pero recomendada)
  -- Aquí verificamos que el usuario esté físicamente cerca (ej. 200 metros)
  -- distance_meters := st_distance(...); -- (Se implementa con PostGIS si lo tienes activo)
  
  -- SI PASA TODO: Registrar la visita
  INSERT INTO public.visits (user_id, partner_id, status)
  VALUES (current_user_id, gym_id, 'approved');

  RETURN json_build_object('success', true, 'message', '¡Bienvenido! Check-in exitoso.');
END;
$$;


ALTER FUNCTION "public"."process_checkin"("gym_id" "uuid", "user_location_lat" double precision, "user_location_long" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."register_new_partner"("gym_name" "text", "gym_address" "text", "gym_category" "text", "gym_lat" double precision, "gym_long" double precision, "gym_price" integer) RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
declare
  new_partner_id uuid;
...
begin
  ...
  -- Lógica de niveles (se mantiene igual)
  IF gym_price < 60000 THEN min_level := 1;
  ELSIF gym_price < 100000 THEN min_level := 2;
  ELSE min_level := 3;
  END IF;

  -- CREAR EL GIMNASIO (Ahora con is_active = FALSE)
  insert into public.partners (
    name, address, category, location, min_plan_level, admin_user_id, daily_capacity_limit, 
    is_active -- <--- Esta columna es la clave
  )
  values (
    gym_name,
    gym_address,
    gym_category,
    st_point(gym_long, gym_lat)::geography,
    min_level,
    current_user_id,
    20, 
    false -- <--- CAMBIO CRÍTICO: Nace desactivado (false)
  )
  returning id into new_partner_id;

  -- Actualizar rol del usuario (se mantiene igual)
  ...
  return json_build_object(
    ...
    'message', 'Solicitud recibida. Pendiente de aprobación.'
  );
end;
$$;


ALTER FUNCTION "public"."register_new_partner"("gym_name" "text", "gym_address" "text", "gym_category" "text", "gym_lat" double precision, "gym_long" double precision, "gym_price" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."reserve_spot"("p_partner_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_user_id UUID := auth.uid();
  ...
  v_count INT;
BEGIN
  -- Validaciones de Plan y Nivel (Igual que antes)...
  SELECT p.access_level INTO v_plan_level
  FROM profiles pr JOIN plans p ON pr.current_plan_id = p.id
  WHERE pr.id = v_user_id AND pr.subscription_status = 'active';

  IF v_plan_level IS NULL THEN RETURN json_build_object('success', false, 'message', 'Sin plan activo'); END IF;

  SELECT min_plan_level, daily_capacity_limit INTO v_min_level, v_capacity
  FROM partners WHERE id = p_partner_id AND is_active = true;

  IF v_plan_level < v_min_level THEN RETURN json_build_object('success', false, 'message', 'Mejora tu plan'); END IF;

  -- Validación de Cupo (Contamos reservas Y completados)
  IF v_capacity IS NOT NULL THEN
    SELECT COUNT(*) INTO v_count FROM checkins 
    WHERE partner_id = p_partner_id AND checkin_date = v_today AND status IN ('reserved', 'completed');
    
    IF v_count >= v_capacity THEN
      RETURN json_build_object('success', false, 'message', 'Cupo lleno para hoy.');
    END IF;
  END IF;

  -- INSERTAR LA RESERVA (Estado: 'reserved')
  INSERT INTO checkins (user_id, partner_id, checkin_date, status) 
  VALUES (v_user_id, p_partner_id, v_today, 'reserved');

  RETURN json_build_object('success', true, 'message', '¡Cupo reservado! Muestra tu QR al llegar.');

EXCEPTION WHEN unique_violation THEN
  RETURN json_build_object('success', false, 'message', 'Ya tienes una reserva activa aquí hoy.');
END;
$$;


ALTER FUNCTION "public"."reserve_spot"("p_partner_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."send_email_resend"("to_email" "text", "subject" "text", "html_body" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  -- ¡¡¡IMPORTANTE!!! PEGA TU LLAVE AQUÍ ABAJO ENTRE LAS COMILLAS
  resend_api_key text := 're_d1WC7hNm_H2PKGmMHd8Ybt22AK2i8uto8'; 
  payload jsonb;
BEGIN
  -- Configuración del paquete
  payload := jsonb_build_object(
    'from', 'RedFit <onboarding@resend.dev>', -- Remitente de pruebas
    'to', to_email,
    'subject', subject,
    'html', html_body
  );
...
ALTER FUNCTION "public"."send_email_resend"("to_email" "text", "subject" "text", "html_body" "text") OWNER TO "postgres";
...
BEGIN
  UPDATE partners SET
    rating = COALESCE((SELECT AVG(rating)::numeric(2,1) FROM reviews WHERE partner_id = COALESCE(NEW.partner_id, OLD.partner_id)), 0),
    review_count = (SELECT COUNT(*) FROM reviews WHERE partner_id = COALESCE(NEW.partner_id, OLD.partner_id))
...
CREATE OR REPLACE FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_partner_admin_id UUID := auth.uid();
  v_partner_id UUID;
  v_checkin_id UUID;
  ...
  v_min_level INT;
BEGIN
  -- 1. Identificar Gimnasio del Admin
  SELECT id, min_plan_level, daily_capacity_limit 
  INTO v_partner_id, v_min_level, v_capacity
  FROM partners WHERE admin_user_id = v_partner_admin_id;

  IF v_partner_id IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'Error: No eres administrador de un gimnasio.');
  END IF;

  -- 2. Buscar Reserva Existente de Hoy
  SELECT id, status INTO v_checkin_id, v_current_status
  FROM checkins 
  WHERE user_id = p_user_id AND partner_id = v_partner_id AND checkin_date = CURRENT_DATE;

  -- CASO A: TIENE RESERVA -> CONFIRMAR INGRESO
  IF v_checkin_id IS NOT NULL THEN
    IF v_current_status = 'completed' THEN
      RETURN json_build_object('success', false, 'message', 'Este usuario YA ingresó hoy (Pase utilizado).');
    ELSIF v_current_status = 'reserved' THEN
      UPDATE checkins SET status = 'completed', created_at = now() WHERE id = v_checkin_id;
      RETURN json_build_object('success', true, 'message', '¡Reserva confirmada! Bienvenido.');
    END IF;
  END IF;

  -- CASO B: NO TIENE RESERVA (Walk-in) -> CREAR INGRESO DIRECTO
  -- Validamos Plan y Aforo antes de dejarlo entrar
  
  -- Validar Plan
  SELECT p.access_level INTO v_plan_level
  FROM profiles pr JOIN plans p ON pr.current_plan_id = p.id
  WHERE pr.id = p_user_id AND pr.subscription_status = 'active';

  IF v_plan_level IS NULL OR v_plan_level < v_min_level THEN
    RETURN json_build_object('success', false, 'message', 'Sin reserva y Plan insuficiente.');
  END IF;

  -- Validar Aforo
  IF v_capacity IS NOT NULL THEN
    SELECT COUNT(*) INTO v_count FROM checkins 
    WHERE partner_id = v_partner_id AND checkin_date = CURRENT_DATE AND status IN ('reserved', 'completed');
    
    IF v_count >= v_capacity THEN
      RETURN json_build_object('success', false, 'message', 'Sin reserva y el gimnasio está lleno.');
    END IF;
  END IF;

  -- Insertar el ingreso directo (Estado 'completed' para evitar el error de constraint)
  INSERT INTO checkins (user_id, partner_id, checkin_date, status)
  VALUES (p_user_id, v_partner_id, CURRENT_DATE, 'completed');

  RETURN json_build_object('success', true, 'message', 'Ingreso directo exitoso (Sin reserva previa).');

EXCEPTION WHEN unique_violation THEN
   RETURN json_build_object('success', false, 'message', 'Error de duplicidad. Intente de nuevo.');
END;
$$;


ALTER FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid", "p_partner_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_profile RECORD;
    ...
    v_checkin_id UUID;
BEGIN
    -- Validar que el perfil exista y la suscripción esté activa
    SELECT * INTO v_profile FROM profiles WHERE id = p_user_id;
    IF v_profile.subscription_status != 'active' THEN
        RETURN jsonb_build_object('success', false, 'message', 'La suscripción no está activa.');
    END IF;

    -- LÓGICA DE TOPES (Actualizada a los 4 planes)
    -- Asignar el tope de visitas según el plan (`current_plan_id`)
    IF v_profile.current_plan_id::text = '1' THEN
        v_max_visits := 15;
    ELSIF v_profile.current_plan_id::text IN ('2', '3') THEN
        v_max_visits := 20;
    ELSIF v_profile.current_plan_id::text = '4' THEN
        v_max_visits := 30;
    ELSE
        RETURN jsonb_build_object('success', false, 'message', 'Plan inválido o no reconocido.');
    END IF;

    -- Calcular visitas usadas durante el ciclo actual
    SELECT COALESCE(COUNT(*), 0) INTO v_visits_used 
    FROM checkins 
    WHERE user_id = p_user_id 
      AND checkin_date >= v_profile.plan_start_date 
      AND checkin_date <= v_profile.plan_end_date;

    IF v_visits_used >= v_max_visits THEN
        RETURN jsonb_build_object('success', false, 'message', 'Has alcanzado el límite mensual de accesos (' || v_max_visits || ').');
    END IF;

    -- Crear registro de checkin "pending"
    INSERT INTO checkins (user_id, partner_id, status)
    VALUES (p_user_id, p_partner_id, 'pending')
    RETURNING id INTO v_checkin_id;

    RETURN jsonb_build_object(
        'success', true, 
        'checkin_id', v_checkin_id,
        'user_name', COALESCE(v_profile.first_name || ' ' || v_profile.last_name, 'Usuario'),
        'user_photo', COALESCE(v_profile.avatar_url, NULL)
    );
END;
$$;


ALTER FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid", "p_partner_id" "uuid") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."checkins" (
    ...
    CONSTRAINT "checkins_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'reserved'::"text", 'confirmed'::"text", 'cancelled'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."partners" (
    ...
    "rate_per_visit" numeric DEFAULT 5000
);
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    CONSTRAINT "profiles_role_check" CHECK (("role" = ANY (ARRAY['user'::"text", 'partner_admin'::"text", 'super_admin'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."subscriptions" (
    ...
    "visits_used_this_cycle" integer DEFAULT 0,
    "created_at" timestamp with time zone DEFAULT "timezone"('utc'::"text", "now"()) NOT NULL,
...
ALTER TABLE ONLY "public"."payments"
    ADD CONSTRAINT "payments_wompi_transaction_id_key" UNIQUE ("wompi_transaction_id");
...
ALTER TABLE ONLY "public"."checkins"
    ADD CONSTRAINT "unique_daily_checkin" UNIQUE ("user_id", "partner_id", "checkin_date");
...
ALTER TABLE ONLY "public"."reviews"
    ADD CONSTRAINT "unique_user_partner_review" UNIQUE ("user_id", "partner_id");
...
CREATE INDEX "partners_geo_index" ON "public"."partners" USING "gist" ("location");



CREATE OR REPLACE TRIGGER "on_reservation_created" AFTER INSERT ON "public"."checkins" FOR EACH ROW EXECUTE FUNCTION "public"."notify_reservation"();



CREATE OR REPLACE TRIGGER "on_review_change" AFTER INSERT OR DELETE OR UPDATE ON "public"."reviews" FOR EACH ROW EXECUTE FUNCTION "public"."update_partner_rating"();



CREATE OR REPLACE TRIGGER "trg_update_partner_rating" AFTER INSERT OR DELETE OR UPDATE ON "public"."reviews" FOR EACH ROW EXECUTE FUNCTION "public"."update_partner_rating"();



ALTER TABLE ONLY "public"."checkins"
    ADD CONSTRAINT "checkins_partner_id_fkey" FOREIGN KEY ("partner_id") REFERENCES "public"."partners"("id");
...
ALTER TABLE ONLY "public"."favorites"
    ADD CONSTRAINT "favorites_partner_id_fkey" FOREIGN KEY ("partner_id") REFERENCES "public"."partners"("id");



ALTER TABLE ONLY "public"."favorites"
    ADD CONSTRAINT "favorites_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "public"."partners"
    ADD CONSTRAINT "partners_admin_user_id_fkey" FOREIGN KEY ("admin_user_id") REFERENCES "public"."profiles"("id");
...
ALTER TABLE ONLY "public"."visits"
    ADD CONSTRAINT "visits_partner_id_fkey" FOREIGN KEY ("partner_id") REFERENCES "public"."partners"("id");



ALTER TABLE ONLY "public"."visits"
    ADD CONSTRAINT "visits_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id");



CREATE POLICY "Acceso a Checkins" ON "public"."checkins" FOR SELECT USING ((("auth"."uid"() = "user_id") OR "public"."is_super_admin"()));



CREATE POLICY "Acceso a Partners" ON "public"."partners" FOR SELECT USING ((("admin_user_id" = "auth"."uid"()) OR "public"."is_super_admin"() OR ("is_active" = true)));



CREATE POLICY "Acceso a Perfiles" ON "public"."profiles" FOR SELECT USING ((("auth"."uid"() = "id") OR "public"."is_super_admin"()));



CREATE POLICY "Cualquiera puede leer reseñas" ON "public"."reviews" FOR SELECT USING (true);



CREATE POLICY "Gimnasio aprueba checkins" ON "public"."checkins" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."partners"
  WHERE (("partners"."id" = "checkins"."partner_id") AND ("partners"."admin_user_id" = "auth"."uid"())))));



CREATE POLICY "Gimnasio ve checkins de su local" ON "public"."checkins" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."partners"
  WHERE (("partners"."id" = "checkins"."partner_id") AND ("partners"."admin_user_id" = "auth"."uid"())))));



CREATE POLICY "Gimnasios son públicos" ON "public"."partners" FOR SELECT USING (true);



CREATE POLICY "Nadie puede cambiar su propio rol" ON "public"."profiles" FOR UPDATE USING (("auth"."uid"() = "id")) WITH CHECK (("role" = ( SELECT "profiles_1"."role"
   FROM "public"."profiles" "profiles_1"
  WHERE ("profiles_1"."id" = "auth"."uid"()))));



CREATE POLICY "Partners can update own gyms" ON "public"."partners" FOR UPDATE USING (("admin_user_id" = "auth"."uid"())) WITH CHECK (("admin_user_id" = "auth"."uid"()));



CREATE POLICY "Permitir a afiliados y partners ver perfiles" ON "public"."profiles" FOR SELECT USING (true);



CREATE POLICY "Planes son públicos" ON "public"."plans" FOR SELECT USING (true);
...
CREATE POLICY "Users can update their own checkins" ON "public"."checkins" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Usuario puede crear checkin" ON "public"."checkins" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuario ve su propia suscripción" ON "public"."subscriptions" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuario ve sus propios checkins" ON "public"."checkins" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios autenticados crean reseñas" ON "public"."reviews" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Usuarios gestionan sus favoritos" ON "public"."favorites" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios ven sus propios pagos" ON "public"."payments" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "super_admin can update partners" ON "public"."partners" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
...
CREATE POLICY "super_admin can update plans" ON "public"."plans" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'super_admin'::"text"))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'super_admin'::"text")))));
...
GRANT ALL ON FUNCTION "public"."can_user_review"("p_partner_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."can_user_review"("p_partner_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."can_user_review"("p_partner_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."confirm_access"("p_checkin_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."confirm_access"("p_checkin_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."confirm_access"("p_checkin_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_checkin_profiles"("p_user_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."get_checkin_profiles"("p_user_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_checkin_profiles"("p_user_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_nearby_partners"("lat" double precision, "long" double precision, "radius_meters" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_nearby_partners"("lat" double precision, "long" double precision, "radius_meters" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_nearby_partners"("lat" double precision, "long" double precision, "radius_meters" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."notify_reservation"() TO "anon";
GRANT ALL ON FUNCTION "public"."notify_reservation"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."notify_reservation"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."partner_scan_user"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."partner_scan_user"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."partner_scan_user"("p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."process_checkin"("p_partner_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."process_checkin"("p_partner_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."process_checkin"("p_partner_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."process_checkin"("gym_id" "uuid", "user_location_lat" double precision, "user_location_long" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."process_checkin"("gym_id" "uuid", "user_location_lat" double precision, "user_location_long" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."process_checkin"("gym_id" "uuid", "user_location_lat" double precision, "user_location_long" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."register_new_partner"("gym_name" "text", "gym_address" "text", "gym_category" "text", "gym_lat" double precision, "gym_long" double precision, "gym_price" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."register_new_partner"("gym_name" "text", "gym_address" "text", "gym_category" "text", "gym_lat" double precision, "gym_long" double precision, "gym_price" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."register_new_partner"("gym_name" "text", "gym_address" "text", "gym_category" "text", "gym_lat" double precision, "gym_long" double precision, "gym_price" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."reserve_spot"("p_partner_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."reserve_spot"("p_partner_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."reserve_spot"("p_partner_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."send_email_resend"("to_email" "text", "subject" "text", "html_body" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."send_email_resend"("to_email" "text", "subject" "text", "html_body" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."send_email_resend"("to_email" "text", "subject" "text", "html_body" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid", "p_partner_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid", "p_partner_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."validate_entry_qr"("p_user_id" "uuid", "p_partner_id" "uuid") TO "service_role";
